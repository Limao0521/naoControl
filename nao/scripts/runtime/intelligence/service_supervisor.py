...
class NemotronServiceSupervisor(object):
    ...
    def __init__(self, base="/home/nao/naoControl", runner=None,
        ...
        self.base = base
        self.pid_dir = posixpath.join(posixpath.dirname(base), "run", "naoControl")
        runtime = posixpath.join(base, "nao", "scripts", "runtime")
        self.start_script = posixpath.join(runtime, "start_nemotron.sh")
        self.stop_script = posixpath.join(runtime, "stop_nemotron.sh")
    ...
    def is_running(self):
        for name in self.SERVICE_NAMES:
            try:
                pid = int(self.read_file(posixpath.join(self.pid_dir, name + ".pid")).strip())
