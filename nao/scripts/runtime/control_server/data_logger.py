...
Parte del backend modular control_server/
"""
...
class SensorReader(object):
    ...
    def get_foot_pressure_data(self):
        data = {}
        data['lfoot_fl'] = self._safe_get_sensor("Device/SubDeviceList/LFoot/FSR/FrontLeft/Sensor/Value", 0.0)
        data['lfoot_fr'] = self._safe_get_sensor("Device/SubDeviceList/LFoot/FSR/FrontRight/Sensor/Value", 0.0)
        data['lfoot_rl'] = self._safe_get_sensor("Device/SubDeviceList/LFoot/FSR/RearLeft/Sensor/Value", 0.0)
        data['lfoot_rr'] = self._safe_get_sensor("Device/SubDeviceList/LFoot/FSR/RearRight/Sensor/Value", 0.0)
        data['rfoot_fl'] = self._safe_get_sensor("Device/SubDeviceList/RFoot/FSR/FrontLeft/Sensor/Value", 0.0)
        data['rfoot_fr'] = self._safe_get_sensor("Device/SubDeviceList/RFoot/FSR/FrontRight/Sensor/Value", 0.0)
        data['rfoot_rl'] = self._safe_get_sensor("Device/SubDeviceList/RFoot/FSR/RearLeft/Sensor/Value", 0.0)
        data['rfoot_rr'] = self._safe_get_sensor("Device/SubDeviceList/RFoot/FSR/RearRight/Sensor/Value", 0.0)
        return data
    ...
    def get_velocity_data(self):
        ...
        try:
            robot_vel = self.motion.getRobotVelocity()
            vx = robot_vel[0] if len(robot_vel) > 0 else 0.0
            vy = robot_vel[1] if len(robot_vel) > 1 else 0.0
            wz = robot_vel[2] if len(robot_vel) > 2 else 0.0
            vtotal = (vx**2 + vy**2) ** 0.5
            return {'vx': vx, 'vy': vy, 'wz': wz, 'vtotal': vtotal}
        except Exception:
            return {'vx': 0.0, 'vy': 0.0, 'wz': 0.0, 'vtotal': 0.0}
    ...
    def get_current_gait_params(self):
        ...
        try:
            # Intentar obtener los parámetros actuales
            config = self.motion.getMoveConfig("Default")
            params = {}
            for key, value in config:
                if key == "StepHeight":
                    params['StepHeight'] = value
                elif key == "MaxStepX":
                    params['MaxStepX'] = value
                elif key == "MaxStepY":
                    params['MaxStepY'] = value
                elif key == "MaxStepTheta":
                    params['MaxStepTheta'] = value
                elif key == "MaxStepFrequency":
                    params['Frequency'] = value
            
            # Rellenar con defaults si faltan
            for key in GAIT_PARAMS:
                if key not in params:
                    params[key] = self.last_gait_params.get(key, 0.0)
